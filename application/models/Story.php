...
class Application_Model_Story extends Zend_Db_Table_Abstract {
    ...
    public function getMostWatchedStories()
    {
        ...
        ->from('categories',array('cat_name' => 'categories.name'))->joinInner('stories', 'cat_id=categories.id')->where('watchSteps<>0')->order('watchSteps DESC')->limit(5);
        $result1 = $this->fetchAll($select)->toArray();

        $select = $this->select()->setIntegrityCheck(false)
        ->from('categories',array('cat_name' => 'categories.name'))->joinInner('stories', 'cat_id=categories.id')->where('watchStory<>0')->order('watchStory DESC')->limit(5);
